const HTML_SOURCE_START_RE = /^\s*(?:<!doctype\s+html\b|<html\b|<head\b|<body\b|<meta\b|<title\b|<link\b|<script\b|<style\b|<div\b|<span\b|<main\b|<section\b|<article\b|<template\b)/i;
const HTML_SOURCE_SIGNAL_RE = /(?:<!doctype\s+html\b|<html\b[\s>]|<\/html>|<head\b[\s>]|<\/head>|<body\b[\s>]|<\/body>|<script\b[\s>]|<\/script>|<style\b[\s>]|<\/style>)/i;
...
export function looksLikeHtmlSource(text = '', context = {}) {
  ...
  const mimeType = String(context.mimeType || '').split(';', 1)[0].trim().toLowerCase();
  ...
  if (/^(text\/plain|application\/octet-stream)?$/i.test(mimeType) && HTML_SOURCE_SIGNAL_RE.test(sample)) {
    return true;
  }
